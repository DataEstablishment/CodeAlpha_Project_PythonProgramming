...
class StockTracker:
    ...
    def __init__(self):
        self.stockstockholdings = {}
        self.price_history = {}

    def add_shares(self, symbol, quantity):
        if symbol in self.stockstockholdings:
            self.stockstockholdings[symbol] += quantity
        else:
            self.stockstockholdings[symbol] = quantity
        print(f"Added {quantity} shares of {symbol} to your tracker.")

    def remove_shares(self, symbol, quantity):
        if symbol in self.stockstockholdings and self.stockstockholdings[symbol] >= quantity:
            self.stockholdings[symbol] -= quantity
            if self.stockholdings[symbol] == 0:
                del self.stockholdings[symbol]
            print(f"Removed {quantity} shares of {symbol} from your tracker.")
        else:
            print(f"Insufficient shares: Cannot remove {quantity} of {symbol}.")

    def view_stockholdings(self):
    ...
    def calculate_total_value(self):
        total_value = 0
        for symbol, quantity in self.stockholdings.items():
    ...
    def track_price_changes(self):
        ...
        for symbol, _ in self.stockholdings.items():
...
def main_menu():
    while True:
        ...
        if choice == '1':
            ...
            print("Current Portfolio:", tracker.view_stockholdings())
